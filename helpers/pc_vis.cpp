...
main (int argc, char** argv)
{
    ...
    cvtColor(left, left_for_matcher, COLOR_BGRA2GRAY);
    cvtColor(right, right_for_matcher, COLOR_BGRA2GRAY);
    ...
    wls_filter->filter(left_disp, left, filtered_disp, right_disp,Rect(), right);
    //! [filtering]
    ...
    getDisparityVis(filtered_disp,filtered_disp_vis,2);
    filtered_disp_vis = filtered_disp_vis(ROI);
    filtered_disp = filtered_disp(ROI);
    left = left(ROI);
  ...
  for(int c = 0; c < left.cols; c++)
  {
    for(int r = 0; r < left.rows; r++)
    {
      ...
      if(filtered_disp.at<int16_t>(r,c)){
        point.z = (615 * 10.f)/filtered_disp.at<int16_t>(r,c) ;
        prev_disp = point.z;
      }
      else{
        point.z = prev_disp;
      }
      point.x = -(c - 320) * point.z/615.f;
      point.y = -(r - 240) * point.z/615.f;
      Vec3b bgr = left.at<Vec3b>(r, c);
      point.r = bgr[2];
      point.g = bgr[1];
      point.b = bgr[0];
